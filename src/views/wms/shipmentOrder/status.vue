      ...
      WmsTable(:data="form.details" @selection-change="handleSelectionChange")
        ...
        el-table-column(label="出库状态" width="150")
          template(slot-scope="{ row }")
            DictSelect(v-model="row.shipmentOrderStatus" :options="row.range" size="small" @change="setShipmentOrderStatus")
...
export default {
  ...
  methods: {
    ...
    setShipmentOrderStatus() {
      this.form.shipmentOrderStatus = this.getShipmentOrderStatus()
    },
    getShipmentOrderStatus() {
      let shipmentOrderStatusArray = []
      this.form.details.map(it => {
        shipmentOrderStatusArray.push(Number(it.shipmentOrderStatus))
      })
      if (shipmentOrderStatusArray.length == 0) {
        return 0
      }
      for (let i = 0; i < 4; i++) {
        if (shipmentOrderStatusArray.every((item) => item == i)) {
          return i
        }
      }
      if (shipmentOrderStatusArray.includes(3) || shipmentOrderStatusArray.includes(2)) {
        if (shipmentOrderStatusArray.includes(0) || shipmentOrderStatusArray.includes(1)) {
          return 2
        }
      }
      if (shipmentOrderStatusArray.includes(1)) {
        return 1
      } else if (shipmentOrderStatusArray.includes(2)) {
        return 2
      } else if (shipmentOrderStatusArray.includes(0)) {
        return 0
      } else if (shipmentOrderStatusArray.includes(3)) {
        return 3
      }
    },
    loadDetail(id) {
